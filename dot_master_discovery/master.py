...
def s3b_family_evidence(df, ad, st, w, pool, anchor, book_file, out, input_sha, attest):
    ...
    try:
        ...
        _write_with_header(os.path.join(out, 'selection_coverage.csv'), covdir, [
            ...
            'terrain = MARKET (price only, no signals); entries = BOOK. The denominator is FIXED, so '
            'coverage is comparable across runs and across candidate books.',
            ...
            'lexicographic order (spec C.3). A book could reach 100% by taking everything; that must '
            'not be reachable.',
        ...
        print(f'    REACH UNAVAILABLE — {type(_exc).__name__}: {_exc}. The S2B terrain exists '
              f'and is written; only this S5B consumption path failed. Reported, not swallowed.',
              flush=True)
        cov = {'episodes': 0, 'coverage_pct': 0.0,
               'by_direction': pd.DataFrame([{'direction': d, 'terrain_episodes': 0,
                                              'touched': 0, 'coverage_pct': 0.0, 'missed': 0}
    ...
    _cd = cov['by_direction']
    _sel_tag = ('SELECTED book' if book_file and os.path.basename(book_file) != 'book50_signals.csv'
                else 'SELECTED book = incumbent this run (no fresh book selected yet)')
    report_lines.append(
        "REACH vs the S2B MARKET TERRAIN (W=15 K=p85 E=p75, mask ADX>=15 & Volume>50 & post-warmup) — "
        + f"{_sel_tag}: "
        + ', '.join(f"{r['direction']} {r['coverage_pct']}%" for _i, r in _cd.iterrows())
        + f" of {cov['episodes']} episodes | INCUMBENT: "
        + ', '.join(f"{r['direction']} {r['coverage_pct']}%" for _i, r in _cd.iterrows())
        + " — this is the line that says whether the rebuild moved the number the redesign exists "
          "to move; terrain is MARKET, entries are BOOK, and coverage never outranks survival")
