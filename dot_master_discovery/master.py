...
def s8b_cluster_profile(df, ad, st, w, pool, anchor, book_file, committed, out, input_sha, attest):
    ...
    with open(tmp, 'w', encoding='utf-8') as f:
        ...
        f.write('# METRIC (g) SUPPRESSED ON BASIS 3: part_pf/part_wr/part_wd/non_pf/non_wr/non_wd are emitted EMPTY for\n')
        f.write('# cluster_basis=3. The basis-3 label is forward-looking, so attributing trade outcomes against it is\n')
        f.write('# circular (a trade entered in direction d inside an episode defined by a forward move in direction d\n')
        f.write('# wins by construction). Basis 3 instead carries COVERAGE ATTRIBUTION (cov_episodes / cov_book_traded /\n')
        f.write('# cov_book_missed / cov_missed_share / cov_book_net), which is non-circular; cov_* is empty on bases 1-2.\n')
