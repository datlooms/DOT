...
**Post-ratification patches (2026-07-18):** master.py received two fixes after the Auditor's PASS on sha aa08a4cceaad — (a) Windows UTF-8 file-I/O (all writes encoding='utf-8'; cp1252 crashed on the '→' char), and (b) natural-sort + S0 header-handling for >9 split parts, plus rebuild.py integration (shared _packutil.py). Current master.py sha: aa08a4cceaad. The committed-path logic is unchanged; $92,347 re-verified REPRODUCED on Windows and from a clean clone. A fresh Auditor pass on db8957587844 is pending to re-bless the new sha.
...
**Final pipeline sha: `master.py aa08a4cceaad`.** Sacred five byte-locked throughout every pass.
