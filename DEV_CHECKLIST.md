...
Sacred five byte-locked: 518862bf19fb / 793e6e5f8d9a / 6530e2508b17 / bb498eb13ce3 / 27af7acee824. **If `core.py` (6530e2508b17) is ever un-sacred'd — after the EA fix and re-export, which is live — it MUST be added to APPENDIX D's lock table: its `capped_poc` partitions price to build the PoC, and Appendix D's coverage of it is incidental to this line, not enumerated there.**
...
**Scope, and the discriminator is dependence on the DISTRIBUTION OF A MARKET QUANTITY** — not constant-versus-percentile, and not population dependence at large. A cut is prohibited outside `dots_thresholds` if placing any single element requires knowing **how a market quantity is distributed across the population**. Percentile, quantile, top-N and bottom-N alike: the prohibition rests on the RANKING REQUIREMENT — to take the top 100 episodes by displacement you must first rank all of them — so "it is a documented constant, not a percentile" is not a defence.

**Depending on the CARDINALITY OR ORDERING OF THE TIME AXIS is not this.** `master.py L337-341` places a day into train-vs-OOS by `n_oos = int(round(n * OOS_TAIL_FRACTION))` and folds by `base = m // FOLD_COUNT`; `wf_selection.py L131/139-140` derives splits the same way. Those need `len(days)` but rank no market value, and they are RATIFIED DESIGN. A prefix or suffix of the time axis — `days[n - n_oos:]` — is permitted for the same reason. Do not re-flag them.

**A rule that classifies each element independently is likewise not prohibited** — `ADX >= 15`, `Volume > 50`, `W = 15`, `CONTIGUOUS_TOLERANCE = 1`, literal bins. `terrain.py L53-56`'s `GRID_W = (15, 30)` and `CONTIGUOUS_TOLERANCE = 1` are documented constants applied directly to market data to define episodes, and they are the ratified terrain construction.

**Mechanism D is the sanctioned route for distribution-dependent cuts precisely because its rolling, day-refreshed window makes the dependence CAUSAL** — the population it consults is the past, not the span. That is why it is the exemption rather than an arbitrary carve-out.

**A rule that classifies each element independently of the others is NOT population-dependent and is NOT prohibited** — `ADX >= 15`, `Volume > 50`, `W = 15`, `CONTIGUOUS_TOLERANCE = 1`, literal bins. This matters: `terrain.py`'s `GRID_W = (15, 30)` and `CONTIGUOUS_TOLERANCE = 1` are documented constants applied directly to market data to define episodes, and they are the ratified design. A constant-versus-percentile rule would have prohibited them.

**Mechanism D is the sanctioned route for population-dependent cuts precisely because its rolling, day-refreshed window makes the dependence CAUSAL** — the population it consults is the past, not the span. That is why it is the exemption, rather than an arbitrary carve-out.
...
**A class no FORM-based rule can see, and it is already prohibited elsewhere.** A constant whose value was read off this span — `disp > 42.7`, where 42.7 was chosen because it split this data nicely — is invisible to any runtime-form test, because at runtime it is indistinguishable from a design choice. It is NOT an uncovered class: it is prohibited by the standing constraint *"all thresholds via `dots_thresholds`"*, which is a separate sentence from the percentile clause and bites independently. What Appendix D adds is a REVIEW OBLIGATION: **a constant applied to a market quantity must state where its value came from — design choice, or number read off this data.** Nothing today distinguishes `TOP_EPISODE_FRACTION = 0.10` chosen by design from the same figure chosen because it fitted this span.

**A third idiom class, documented for review rather than enforced.** Beyond `np.percentile`-family and sorted-index forms there is `pd.cut` / `pd.qcut` / `np.digitize`. One live site: `family_evidence.py L280`, `pd.cut` on cluster size with hardcoded literal bins — sanctioned on BOTH stages: each element maps to a band independently of the others (not distribution-dependent), and cluster size is a book quantity. `pd.qcut` — a data-derived quantile cut in a single call — appears nowhere in the package. **Verified at HEAD `39fbd4d`: one `pd.cut` site, zero `pd.qcut`, zero `np.digitize`.** Unlike every other claim in this section, which the sha lock re-verifies each run, this one is verified once — so **re-running this sweep is part of the lock-manifest review**, and that review owns it. This class is NOT added to any enforcement sweep: that would repeat the `sorted(` volume mistake for no gain. It is named here because `family_evidence.py` is edited by this build and therefore unlockable, so a contributor writing `pd.qcut(episode_disp, 10)` there would evade both the token form and the file lock. That is a concrete instance of the residual class above, not an exception to it.

**Dependency, not a footnote: `core.py`'s `capped_poc(H, L, C, V, bucket)` partitions price to build the PoC — a fifth market-object definer.** It is protected today only because `core.py` is one of the sacred five (`6530e2508b17`). That coverage is INCIDENTAL, not enumerated here. **If `core.py` ever loses sacred status — after an EA fix and re-export, which is a live prospect — it joins this lock table**, or Appendix D's enumeration silently reads four modules while five exist.
...
| `concurrence_profiler.py` | 841 | `np.argsort(-J[iu])[:TOP_PAIRS_N]` — top-N over a Jaccard matrix, emitted to CSV. **Distribution-dependent** (it needs the ranking), so it is sanctioned by STAGE (ii) alone: the objects are condition pairs, not market data. Not by the fixed-N argument, which does not survive |
