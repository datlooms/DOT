...
5. Per-signal terrain coverage against BOTH denominators, pinned cell W15/K85/E75 named in every column: raw terrain (7,490 episodes) and REACHABLE (episodes holding at least one eligible bar where D2D agrees with episode direction). Reachable is the primary figure; abort the run if any episode threshold is a local percentile rather than mechanism D.
6. Emit the unclaimed-reachable set — reachable episodes no catalogue signal touches, with direction, duration, displacement and time-of-day. This is the visibility deliverable: the map of what is legitimately available and currently unoccupied.
...
- WHOLE CAKE = the reachable universe, not all price action. Measured decomposition: all episodes 100% -> eligible (ADX>=15, ticks>50, post-warmup) 60.5/60.8% -> D2D agrees 30.0/31.4%, and ANY of the 249 conditions fires on 30.0/31.4%. The vocabulary loses nothing; the two exclusions are deliberate, measured decisions (gating PF 16.63 vs 2.25 OOS; D2D gated PF 5.14 vs long-ungated 1.53).
- BOOK-50 occupies 1.415%/0.762% of raw terrain = 5.0%/2.4% of reachable. The constraint is signal count, not vocabulary, not gates, not search.
