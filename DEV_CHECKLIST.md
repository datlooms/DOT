...
**Scope, and the discriminator is POPULATION DEPENDENCE — not constant-versus-percentile.** A cut over MARKET DATA is prohibited outside `dots_thresholds` if **placing any single element requires knowing the rest of the population**. Percentile, quantile, top-N and bottom-N alike: a fixed top-N and a fixed top-p% are the same operation, since `N / total = p`, so "it is a documented constant, not a percentile" is not a defence — to take the top 100 episodes you must first have all episodes.
...
| `concurrence_profiler.py` | 841 | `np.argsort(-J[iu])[:TOP_PAIRS_N]` — top-N over a Jaccard matrix, emitted to CSV. **Population-dependent** (it needs the ranking), so it is sanctioned by STAGE (ii) alone: the objects are condition pairs, not market data. Not by the fixed-N argument, which does not survive |
